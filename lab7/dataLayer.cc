...
//    }
    ...
    if(msg->getArrivalGate()==fromApp){
        ...
        N_PDU *apkt = dynamic_cast<N_PDU *>(msg);
        if(apkt){
            int seq= apkt->getID();
           ...
           cancelEvent(event);
            ...
            DL_PDU *dpkt = new DL_PDU("rpdu");
            dpkt->setType("Data");
            dpkt->setID(-1);
            // SET TO -1 BECAUSE making no use of them in stop and wait
           dpkt->setSourceAdd(-1);
           dpkt->setDestiAdd(-1);
            dpkt->encapsulate(dynamic_cast<cPacket *>(msg));
            message=dpkt;
